...
<!-- knitr::purl("./FW670/BigPicture.qmd", output="./FW670/BigPicture_code.R") -->
...
```{=html}
<details>
<summary>Is this a hypothesis?</summary>
```
...
::: {.fragment}
```{=html}
<span style="color:#FF0000";><b>Non-hypothesis hypothesis:</b></span>  We hypothesize there to be a lot of variation in coyote home range size.
```
:::
...
2. <span style="color:red">Better</span>: Coyote home ranges in urban areas with high available food resources are smaller than coyote home ranges in urban areas with low available food resources and smaller than coyotes living in non-urban areas
...
["*The hidden Biases of Big Data*" by Kate Crawford  (2013)](https://hbr.org/2013/04/the-hidden-biases-in-big-data)
...
![](../img/dataspeak.png){fig-align="center" width="1200"}
<br>

[- Correia et al. 2026. Best practices for moving from correlation to causation in ecological research. Nature Communications.](https://www.nature.com/articles/s41467-026-69878-z)
...
[**Bradley et al. 2021 (Nature)**](https://www.nature.com/articles/s41586-021-04198-4)

<!-- (../Information_files/survey.data.png){fig-align="center" width="1000"} -->
...
```{=html}
<b>Statistical thinking: </b> data contain <span><u>information</u></span>, depending on <br>
```
...
-   apply learning outside of the data to a 'population' (<span style="color:red">inference</span>)
...
What is the population mean for this truth?
...
What is the theoretical population mean that generated these values?

. . .

$$
E[Y] = \sum_{i=1}^{N} y\times P(Y=y) = \sum_{i=1}^{N} y \times 1/N \\
$$
...
### Estimator for this truth's population mean
...
-   <span style="color:red">Key Weaknesses</span>: 
    - Can be difficult to assess assumptions
    - Sampling frame is not always clear and thus the population you are infering to is not entirely clear


## Model-Based

[Wikipedia link for Poisson](https://en.wikipedia.org/wiki/Poisson_distribution)
...
<!-- ## Many samples per Population -->

<!-- ```{r,echo=TRUE} -->
<!-- n.sim = 10000 -->
<!-- samples.list = lapply(list.mat,FUN=function(x){matrix(replicate(n.sim, -->
<!--                                                                 sample(x,size=n) -->
<!--                                                                 ), -->
<!--                                                       nrow=n -->
<!--                                                       )}) -->
<!-- length(samples.list) -->
<!-- dim(samples.list[[1]]) -->
<!-- lambda.hat = lapply(samples.list, -->
<!--                     FUN=function(x){apply(x,1,mean)}) -->
<!-- lambda.hat=unlist(lambda.hat) -->
<!-- length(lambda.hat) -->
