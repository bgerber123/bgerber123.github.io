...
<!-- knitr::purl("./FW670/glm1.qmd", output="./FW670/glm1.R") -->
...
-   design matrix of the known explanatory variables ($\textbf{X}$)
...
<!-- BDG: Note that this fragment fails in creating a pdf if the equations are together -->
...
n = sample size
$x_{2}$ & $x_{3}$ are independent variables
...
Optimization algorithims do not like boundaries (e.g., 0 or 1). It's easier to **guess** values to evaluate in a maximum likelihood optimization when there are no bounds. ($-\infty$, $\infty$)
