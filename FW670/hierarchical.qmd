...
<!-- knitr::purl("./FW670/hierarchical.qmd", output="./classfiles/glmmlab/hierarchical.R") -->
...
\begin{align*}
y_i \sim& \text{Poisson}(\lambda_i)\\
\text{log}(\lambda_i) =& \beta_0 + \beta_1\times \text{veg}_{1} + \beta_2\times \text{veg}_{2}+ \beta_3\times \text{veg}_{3}
\end{align*}
$$
...
\text{log}(\lambda_{i}) =& \mu + \beta_{j[i]}\\
 \beta_{j} \sim& \text{Normal}(0, \sigma^2_{\beta})
...
New hypothesis: Leopard detection varies by cover and vegetation, where the effect of cover comes from a shared distribution
...
\begin{align*}
y_{i} \sim& \text{Poisson}(\lambda_{i})\\
\text{log}(\lambda_{i}) =& \mu_{\text{veg}} + \beta_{\text{veg}_{j[i]}} + \mu_{\text{cov}}+ \beta_{\text{cov}_{j[i]}}\\
 \beta_{\text{veg}_j} \sim& \text{Normal}(0, \sigma^2_{\text{veg}})\\
 \beta_{\text{cov}_j} \sim& \text{Normal}(0, \sigma^2_{\text{cov}})
