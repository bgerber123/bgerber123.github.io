...
## Do we need to design study's anymore to do research?
...
```{=html}
<br><span style="color:#FF0000";><center><em>"they are creations of human design."</center></em></span>
```
...
```{=html}
<br><center><span style="color:#FF0000";><em>"The Big Data Paradox: the bigger the data, the surer we fool ourselves”</em></span>  when we fail to account for our sampling process. </center>
```
...
## The Questioning Scientist {.smaller}

<br>

**My opinion:** In regard to *data* and *statistical models*, [21^st^ century scientists should be pragmatic, excited, and questioning.]{style="color:red"}
...
![](Information_files/banding.png){fig-align="center"}
...
**Fisher Information:** measures the information an observable random variable (**Y**) has about an unknown parameter $\mathbf{\theta}$ upon which the probability of Y (*f*(**Y**;$\mathbf{\theta}$)) depends.
...
To learn about $\mathbf{\theta}$ from **Y**, we need to link them together via *f*(**Y**;$\mathbf{\theta}$)
...
-   A probability function for **Y**:

    -   *f*(**Y**;$\mathbf{\theta}$)

    -   \[**Y**\|$\mathbf{\theta}$\]

-   The unknown parameter(s): $\mathbf{\theta}$, which are specified in the probability function
:::
...
**Y** is a random variable as it depends on random events.
...
Let's assign a probability function, f(Y;$\mathbf{\theta}$), which describes the probability for each i^th^ data (y~i~). This is not just for the data we observed, but for all possible data that could be observed.
...
Now we know that we have to assign a probability to only two possible outcomes of the random variable Y.
...
```{=html}
<center>
This probability mass function (PMF) is called ________?
</center>
```
## Probability Function
...
-   q = 1 - $\theta$; thus, $0 \leq q \leq 1$

-   p + q = $\theta + 1-\theta = 1$

    -   law of total probability is satisfied
:::
...
Let's sum the successes of the plots with our plant and divide by the total number of plots. Intuitive?
...
Let's combine 1 observation of our random variable y, our PMF, and $\theta$ to quantify the Fisher information of **Y**:
...
[Link](https://en.wikipedia.org/wiki/Fisher_information#Single-parameter_Bernoulli_experiment)
...
Let's consider how information varies by $\theta$ for a given sample size....
...
If you were designing a study, what would you like $\theta$ to be and how would you control it?
...
```
Undergraduate statistics classes only focus on Power Analysis.
```{=html}
...
-   what parameter we want to estimate and the PDF/PMF
-   the goal of the question
:::
