...
Please create a markdown document (.Rmd) that is output as an html (.html) that implements the R code outlined below along with text explanation of what the code is doing (i.e., annotation). Practice good code structure using indentation. 

<span><u>Create at least one equation of your choosing.</u></span>

## <span style="color:blue">What to include</span>

### <span style="color:red">Create Data</span>

- Simulate values of a random variable `y` from any probability distribution (e.g., `rnorm`, `rgamma`,`rbeta`,`rexp`,`runif`), creating a matrix (function name) that is 1000 rows by 200 columns.

### <span style="color:red">apply function</span>

- use the `apply` function to calculate the `range` (function name) of values for each row.
