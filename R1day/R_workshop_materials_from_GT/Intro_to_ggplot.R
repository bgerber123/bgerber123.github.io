...
# > alter the theme
# > edit the x axis (25 hours?)
# > facet by species
# > consider that time of day is circular
...
ggplot(sightings, aes(x=hour, y=count))+
  scale_x_continuous(breaks = c(1, 3, 6, 9, 12, 15, 18, 21, 24))+
  geom_rect(aes(xmin = 0, xmax = 6, ymin = 0, ymax = max(count) + 1), fill="gray80", alpha = 0.02)+
  geom_rect(aes(xmin = 18, xmax = 24, ymin = 0, ymax = max(count) + 1), fill="gray80", alpha = 0.02)+
  geom_jitter(width = 0.2, height=0.2, alpha=0.3, aes(col = species))+
  geom_smooth()+
  labs(x="Hour of the day", y="Count of individuals \n per sighting")+
  theme_classic(base_size = 14)
...
ggplot(sightings, aes(x = hour, y=count))+
  geom_col(aes(fill = species), position = "stack") # options are stack, dodge, fill
...
### Challenge: change the color scheme
...
sightings %>%  
  ...
  geom_boxplot(aes(fill=habitat))+
  annotate("text", x = 2, y = 6, label = "p < 0.01")+
  annotate("text", x = 3, y = 8.5, label = "p = 0.34")
...
### Finally, we will explore ridgeplots, which are very useful for visualizing data distributions:
...
ggplot(sightings, aes(x = hour, y=species))+
  geom_density_ridges(aes(fill=habitat, col=habitat),
                      alpha=0.2,
                      rel_min_height=0.02)+
  theme_classic()+
  scale_fill_manual(values = c("darkgreen","greenyellow","brown"))+
  scale_color_manual(values = c("darkgreen","greenyellow","brown"))+
  scale_x_continuous(limits = c(0, 24), breaks=c(1,3,6,9,12,15,18,21,24))
