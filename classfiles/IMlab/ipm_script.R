#################################################################################
######################### FW680A4 10/22/24 Example ##############################
######################## Integrated Modeling - IPM ##############################
#################################################################################
...
# 1. What line number(s) is where demographic stochasticity is incorporated into the model?


# 2. What does the last column in an m-array represent?


# 3. What does tau on line 96 represent?


# 4. Interpret line 118 in words:       
# pr.j[t,j] <- sj[t] * prod(sa[(t+1):j]) * prod(q[t:(j-1)]) * p[j]
...
## Modify the model (and associated JAGS data file) to incorporate a covariate of May rainfall on productivity of woodchat shrikes
...
## Modify the model to make recapture probability differ for juveniles and adults. 
## Will the data support this much heterogeneity in the model? 
## How can you tell?
