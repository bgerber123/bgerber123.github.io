...
# Objective: To investigate the influence of the Prior on the posterior distribution.
...
# The set.seed makes sure the random sample is the same every time the code is run
...
# We do this by searching over different values of mu and want to find the value of mu (mean) that maximizes the probability
# of observing the data we observed. Or, the likelihood of the data.
  ...
  for(i in 1:length(possible.mu)){
    save.likelihood[i] = prod(dnorm(sample.weights, 
                                    possible.mu[i], 
                                    sigma1
                                    )
                              )
...
##############################################################
# Lets now move to the Bayesian world and specify prior information about our unknown parameter
# that we want to estimate.

# First, lets consider specifying a diffuse prior- where the probability density is spread 
# over a very large region of parameter space - meaning lots of possible paramter values have 
# a small amount of information

# Normal prior hyper-parameters
  prior_mu = 0
...
# sample draws from the posterior distrubtion so to visualize and use the posteriod distribution.
  ...
  postdist = PD_mu(prior_mu = prior_mu, 
