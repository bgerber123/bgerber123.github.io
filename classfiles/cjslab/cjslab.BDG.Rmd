...
```{r echo=TRUE,eval=TRUE}
# The probability of a sex effect- that female survival is less than male survival
# To evaluate this, we want to know if beta1 is negative  
length(which(beta1<0))/length(beta1)
```

The probability that $\beta_1 < 0$ indicates there is no evidence of a difference. A high probability, values near 1, would indicate a statistical clear negative effect. However, if we saw probabilities near 0, this would indicate a clear statistical positive effect difference between female and male survival probability.
