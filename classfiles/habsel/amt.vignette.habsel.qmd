...
### Fit the same model with the glm function

```{r glm}
  rsf2 = glm(case_~forest, data=rsf1,family=binomial(link="logit"))
  summary(rsf2)

```

### Plor Predicted Relative Selection

```{r pred}
  rsf2 = glm(case_~forest, data=rsf1,family=binomial(link="logit"))
  X=model.matrix(~forest,data = rsf1)
  
  preds.link = predict(rsf2,newdata=data.frame(forest = values(sh_forest)),
                       type="link")
...
rsf.data1 <- random_points(hr, n = 50, presence = deer) |> 
  extract_covariates(sh_forest) 

head(rsf.data1)
table(rsf.data1$case_)

rsf.fit1 = glm(case_~forest, data=rsf.data1,family=binomial(link="logit"))
summary(rsf.fit1)
