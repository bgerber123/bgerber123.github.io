...
# with differing detection functions- is abundance different by a lot or a little or not at all.
  ...
  model1b = secr.fit(input.adu.annual, 
                     model=list(D~1,g0~1, sigma~1),
                     detectfn=2, 
                     mask=mask.adu, 
                     sessioncov = session_cov,
...
# Plot the detection function
  plot(model1,xval=0:5000,lwd=4,ylim=c(0,0.05))
  plot(model1b, xval=0:5000,lwd=4,col=2,add=TRUE)

  N.hat.model1 = region.N(model1, region = mask.adu, session = 1, se.N = TRUE, alpha = 0.05, loginterval = TRUE)
  N.hat.model1b = region.N(model1b, region = mask.adu, session = 1, se.N = TRUE, alpha = 0.05, loginterval = TRUE)
...
# Let's consider a third model that has density vary by sex
model3 = secr.fit(input.adu.annual, 
                  model=list(D~Sex*Percent,g0~Sex, sigma~Sex),
                  detectfn=0, 
                  mask=mask.adu, 
                  sessioncov = session_cov,
                  method ="Nelder-Mead",start=model3)
...
N.hat.F2 = region.N(model3, region = mask.adu, session = 1, se.N = TRUE, alpha = 0.05, loginterval = TRUE)
N.hat.M2 = region.N(model3, region = mask.adu, session = 2, se.N = TRUE, alpha = 0.05, loginterval = TRUE)

N.hat.F2
N.hat.M2


pred.Dsurface2 <- predictDsurface(model3, mask.adu)
par(mfrow=c(2,1))
plot(pred.Dsurface2[[1]], title="Female")
plot(pred.Dsurface2[[2]], title="Male")
