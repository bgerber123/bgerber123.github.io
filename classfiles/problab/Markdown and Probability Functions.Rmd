...
Markdown is quite useful for developing reports or doing homeework. Anytime you want to combile code, plotting, or analyses with text, markdown can help. We will use markdown as a way accomplish homework. This way you can provide code and document it simultaneously. I would suggest to not try and get too fancy at first. Keep it simple. Write-text in RStudio as you would in any word processor. Write R code in R chunks. Use "echo=FALSE" and "echo=TRUE" for a code chunk to either not print the code or to print the code. 
...
But what is the *expected value*? Summarizing [wikipedia](https://en.wikipedia.org/wiki/Expected_value), the expected value (also called expectation, expectancy, expectation operator, mathematical expectation, mean, expectation value, or first moment of a probability function) is a generalization of the weighted average.

Informally, the expected value is the arithmetic mean of the possible values a random variable can take, weighted by the probability of those outcomes. For symmetric distributions, like the Normal, the expected value is in the middle of the distribution, thus it is also the mode probable (i.e. mode). This is not the case for non-symmetric distributions (i.e., left side of the distribution has a long tail and the right side does not have the same long tail).
