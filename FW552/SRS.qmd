...
ponds.all$Abs.Sample.Error = abs(ponds.all$Sample.Mean-8)
...
Var Sample Means: `r N=6; mean ( (N-2)/N * var.per.sample/2 )`


## [Sample Distribution of Variance]{style="color:#078BCD;"}
...
$$
E[\text{Sampling Distribution Variance}] = \\\frac{1}{n} \left(\frac{N-n}{N}\right) \times \sigma^2
$$

- As the sample size (n) increases the sample variance declines by **1/n**
- Finite-population correction factor (N-n/N)
...
- E[$\sigma^2$] $\times$ 1/n $\times$ FCF = **4.27**
...
**If you know the E[$\sigma^2$]** ...

- you have a mechanism to understand the variation of sample distribution of the means
- the more variation in $y$, the more variation in sampling distrubution of means ($\hat{\mu}$.
...
- research objective probably impacts $n$ the most
