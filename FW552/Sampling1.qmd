...
- **What is sampling?**
    - The process of selecting a part of a population to be used to estimate characteristics of the population
...
- [Convenient sampling]{style="color:cornflowerblue"} 
- [Judgment sampling]{style="color:cornflowerblue"} 
- [Probability sampling]{style="color:cornflowerblue"} 

<!-- - when most convenient units are chosen -->
<!-- obtained at the discretion of someone who is familiar with the characteristics of the population -->
<!-- - based on a process that the probability a unit is selected is known  -->
...
- Surveys in 1) The Savannah River Site (green) 2)  Newberry County (blue), 3) Hampton County (red)
...
## [Generalizability?]{style="color:#078BCD;"}
...
- I will take what I learned and apply to a larger area that is the *Population*
...
E.g., Nesting shorebirds are censused each summer at a few beaches.
...
$\textbf{y} = \begin{bmatrix} 69 & 54 & 72 & 61 & 58 & 71 \end{bmatrix}$
...
$$\hat{\mu} = \left(\sum_{i=1}^{n}y_{i}\right)\times \frac{1}{n} = 57.7$$
...
$$
\text{Bias}(\hat{\mu},\mu) = \left(\int_{\hat{\mu}} \hat{\mu} p(\hat{\mu})dx\right) - \mu
$$

. . .

This expectation is an average over all samples of size $n$.
...
[A measure of how close the average sample value is to truth]{style="color:cornflowerblue"}
...
## [Estimator Consistency]{style="color:#078BCD;"}

An estimator of a parameter is *consistent* if it converges in probability to the true value of the parameter as the sample size tends to infinity.
