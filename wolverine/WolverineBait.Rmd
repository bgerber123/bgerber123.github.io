...
The second part is more challenging. This is a more insidious problem. We can investigate this by conditioning on sites that were occupied in 2017 and asking if the change in occupancy (colonization/extirpation) is different based on whether a site was baited both years or used lure both years or changed from bait to lure. Unfortunately, no sites that used lure in 2017 were then baited in 2022. There should not be a difference in colonization or extirpation  based on whether the bait-lure status changed. Sites should be estimated to be occupied, or not, and/or changed their occupancy status independent of bait-lure status.

However, we have to think about confounding factors between a bait site and a lure site. One way we can think of this is via covariate comparisons between these site types, which is done below. But there are of course unmeasured factors that we can not do this for. If a site was chosen to be a bait site in both survey years because of a certain factor that is different than a site that had bait (2017) and then had lure (2022) we could expect this difference to be detected in the analyses below but is not a signal of an effect of bait but because of this confounding factor. This is a concern. 

This point is captured by Justin Gude: "Our ability to disentangle the effect on detection method of using baited versus scent-pump only sites on occupancy [not detection] is hampered by different site type selection methods among states. Washington and Montana switched from largely baited sites in 2017 (other than inaccessible sites) to exclusively scent-pump only sites in 2022. Idaho and Wyoming used both types of sites in both surveys. However, the use of baited sites in these states was in order to collect genetic material, so they placed these sites in areas known to previously and consistently contain wolverines, to maximize success in collecting genetic material. So wolverine occurence at these baited sites in 2022 may have inherently been higher."
...
#  visitData2022$SiteType2022[index]="Lure"
  ...
  visitData2022$SiteType2022[index]="Bait"
    ...
    index=which(habData2022$SiteType2022=="Bait_Lure")
  habData2022$SiteType2022[index]="Bait"
```
...
<span style="color:purple">Summary: I believe there are dynamics going on at all sites, probably due to wolverines moving around and sampling variation. However, there seems to be a *suggestion* of an effect of detection method. Sites with bait in both years have a moderate increase in mean occupancy, while when changing to lure from bait, occupancy decreases. We would expect no change in a site that uses lure in both years and that is exactly what we see. </span>

<span style="color:purple">The fundamental question because bait/lure site designations were not randomized, is what is different between these bait sites and lure sites in 2017 and different between Bait-->Bait and Bait-->Lure sites? It could be a detection method difference, which is a concern. OR, it could be a confounding factor, for example, that Bait-->Bait sites were chosen because the state sampling also wanted genetic material and chose site to deploy bait at sites known to have wolverines for a long time. Note that when we are examining Bait-->Bait sites, we are only examining sites in Wyoming and Idaho. When we are examining Bait-->Lure sites, we are looking at sites from Idaho, Montana, Washington, and Wyoming. Should we expect different dynamic parameters (by BaitBait, BaitLure) because of the locations of these two samples?</span>

<span style="color:purple">My overall take away: There is a concern (not perfect evidence) that lure sites are missing wolverine. Not a huge concern, but something to note is that occupied sites have a lower detection probability if using lure only (clear evidence).</span>


<span style="color:purple">1. What to do next: Consider randomizing a selection of sites that went from Bait-->Lure to be sampled using bait in the third survey. If there is a detection method affect on occupancy we should expect a high proportion of these sites to become occupied or occupied at a higher rate than Bait-->Lure sites that continued to be a lure site in the third survey. </span>


<span style="color:purple">2. What to do next: Either include this analyses in the 2nd manuscript as evidence of the concern or to present the concern in the discussion logically without the analysis. The later is my preference to keep the manuscript from getting too overwhelming (three occupancy analyses using the same data in one manuscript). Note that we have at least partly ameliorated the effect issue of detection method by including bait/lure covariate on the detection probability in the 2017 and 2022 analyses. Because of this change, the difference in occupancy across the years is less than it is when not including it. We can not fix the full detection method concern on occupancy, but we can discuss it in the manuscript without this analysis presented above.  </span>
