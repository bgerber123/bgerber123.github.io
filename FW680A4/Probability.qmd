...
y.mat = replicate(1000,
                  rbinom(n = 10,size=N,theta)
                  )
```

```{r bern3, echo=FALSE, eval=TRUE,  out.width="75%"}
hist(apply(y.mat, 2, mean),freq=TRUE,breaks=40, main=bquote("Sampling Distribution of"~theta),xlab=expression(theta))
```
...
curve(dnorm(x, mean = 20, sd = 1),xlim=c(0,40),lwd=4,col=3,add=TRUE)
```
